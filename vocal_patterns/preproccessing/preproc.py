...
def process_audio(y, sr):
        ...
        if current_length_samples > target_length_samples:
            ...
            padded_signal = librosa.util.pad_center(y, target_length_samples + padding_samples)
...
def mel_spectrogram(y_trunc, sr):
    ...
    db_spectrogram = librosa.display.specshow(librosa.power_to_db(mel_spectrogram, ref=np.max), y_axis='mel', x_axis='time')
